...
func apiRoutesRegister(route *gin.RouterGroup) {
	...
	apiRoutes.PUT("/delete-groups", GroupsController.Delete).Use(Auth.TokenAuth())
	apiRoutes.PUT("/enable-peers", PeersController.Enable).Use(Auth.TokenAuth())
	apiRoutes.PUT("/enable-users", UserController.Enable).Use(Auth.TokenAuth())
	...
	apiRoutes.POST("/initialState", SystemController.InitialState).Use(Auth.TokenAuth())
